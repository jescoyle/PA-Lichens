...
## Creates a unique plot id number for FIA/FHM data
make.plotid = function(x){
  paste(x$STATECD, x$COUNTYCD, x$PLOT, sep='_')
}

## Creates a unique sampling id number for FIA/FHM data
make.yrplotid = function(x){
  paste(x$MEASYEAR, make.plotid(x), sep='_')
}	

## Combines abundance classes when taxa are lumped within an FIA plot
## This happens when updating taxonomy.
