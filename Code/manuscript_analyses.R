...
fia_sptab$yrplotid <- fia_lichen$yrplotid
eco_L2$fiarecs <- sapply(eco_L2$L2CODE, function(x) nrow(subset(fia_sptab, L2CODE==x)))
eco_L2$fiaplots <- sapply(eco_L2$L2CODE, function(x) length(unique(subset(fia_sptab, L2CODE==x)$yrplotid)))
...
# This set excludes Genus-only records when there are congenerics on the same plot
fia_splist_nogenus <- sapply(fia_plots$yrplotid, function(y){
	these_sp <- get_species(subset(fia_lichen, yrplotid==y)$Binomial)
	these_sp[order(these_sp)]
})

# This set includes Genus-only records when there are congenerics on the same plot
...
## Calculate species richness in each plot
# This set excludes Genus-only records when there are congenerics on the same plot
inv_splist_nogenus <- sapply(inv_plots$Site_Number, function(y){
...
dimnames(inv_splist_nogenus)[[2]] <- inv_plots$Site_Number

# This set includes Genus-only records when there are congenerics on the same plot
inv_splist <- sapply(inv_plots$Site_Number, function(y){
  these_sp <- subset(inv_lichen, Site_Number==y)
  
  all <- unique(these_sp[these_sp$all,]$Binomial)
  all <- all[order(all)]
  
  epi <- unique(these_sp[these_sp$epi,]$Binomial)
  epi <- epi[order(epi)]
  
  macro <- unique(these_sp[these_sp$macro,]$Binomial)
  macro <- macro[order(macro)]
  
  epi_macro <- unique(these_sp[these_sp$epi_macro,]$Binomial)
  epi_macro <- epi_macro[order(epi_macro)]
  
  list(all = all, epi = epi, macro = macro, epi_macro = epi_macro)
})
dimnames(inv_splist)[[2]] <- inv_plots$Site_Number

inv_S <- t(apply(inv_splist, 1:2, function(x) length(x[[1]])))
colnames(inv_S) <- paste('S', colnames(inv_S), sep='_')

inv_plots <- cbind(inv_plots, inv_S[as.character(inv_plots$Site_Number),])
...
# Save
#write.csv(table1, file.path(results_dir, 'Table_1_datasets.csv'), row.names=F)
...
# Make figure
pdf(file.path(fig_dir, 'Fig_2-richness_distributions.pdf'), height=3, width=4)
par(mar=c(4,4,1,1))
par(lend=1)
...
#svg(file.path(fig_dir, 'Fig_2-richness_distributions.svg'), height=3, width=4)
par(mar=c(4,4,1,1))
par(lend=1)
make_plot(c(0,33), c(0, 25), xlab='Number of Species', ylab='Number of Plots')
points((0:33)-jit, inv_tab, type='h', lwd=2, col=data_cols['INV'])
...
species_fia <- unique(fia_lichen$Binomial)
species_fia <- species_fia[order(species_fia)]

fia_siteXsp = t(sapply(fia_splist, function(x) species_fia %in% x))
colnames(fia_siteXsp) = species_fia

species_inv <- unique(inv_lichen$Binomial)
species_inv <- species_inv[order(species_inv)]

inv_siteXsp <- apply(inv_splist, 1:2, function(x) species_inv %in% x[[1]])
dimnames(inv_siteXsp)[[1]] <- species_inv
inv_siteXsp <- aperm(inv_siteXsp, c(2,3,1))


# Save site x species matrices for later
#save(fia_siteXsp, inv_siteXsp, file = file.path(results_dir, "siteXspecies_matrices.RData"))


## Rarefaction of species richness
## Note: Genus-only taxa included
...
fia_acc <- by(fia_siteXsp[fia_plots$yrplotid,], fia_plots$L2NAME, function(x) specaccum(x, method='exact'))
inv_em_acc <- by(inv_siteXsp['epi_macro', as.character(inv_plots$Site_Number),], inv_plots$L2NAME, function(x) specaccum(x, method='exact'))
inv_all_acc <- by(inv_siteXsp['all', as.character(inv_plots$Site_Number),], inv_plots$L2NAME, function(x) specaccum(x, method='exact'))
inv_epi_acc <- by(inv_siteXsp['epi', as.character(inv_plots$Site_Number),], inv_plots$L2NAME, function(x) specaccum(x, method='exact'))
inv_macro_acc <- by(inv_siteXsp['macro', as.character(inv_plots$Site_Number),], inv_plots$L2NAME, function(x) specaccum(x, method='exact'))
...
fia_rich <- by(fia_siteXsp[fia_plots$yrplotid,], fia_plots$L2NAME, specpool)
inv_em_rich <- by(inv_siteXsp['epi_macro', as.character(inv_plots$Site_Number),], inv_plots$L2NAME, specpool)
inv_all_rich <- by(inv_siteXsp['all', as.character(inv_plots$Site_Number),], inv_plots$L2NAME, specpool)
inv_epi_rich <- by(inv_siteXsp['epi', as.character(inv_plots$Site_Number),], inv_plots$L2NAME, specpool)
inv_macro_rich <- by(inv_siteXsp['macro', as.character(inv_plots$Site_Number),], inv_plots$L2NAME, specpool)
...
#write.csv(as.matrix(rich_ests_all), file.path(results_dir, 'richness_estimators_compared.csv'), row.names=F)
...
#write.csv(chao_tab, file.path(results_dir, 'Table_2-chao_estimator.csv'), row.names=F)
...
# Drop Genus-only taxa from both datasets unless there are no congenerics
keep_taxa <- get_species(c(colnames(fia_siteXsp), dimnames(inv_siteXsp)[[3]]))

# Species occurence within regions
fia_occ <- by(fia_siteXsp[fia_plots$yrplotid, colnames(fia_siteXsp) %in% keep_taxa], fia_plots$L2NAME, function(x) colSums(x)[colSums(x)>0])
inv_em_occ <-  by(inv_siteXsp['epi_macro', as.character(inv_plots$Site_Number), dimnames(inv_siteXsp)[[3]] %in% keep_taxa], 
                  inv_plots$L2NAME, 
                  function(x) colSums(x)[colSums(x)>0])
inv_macro_occ <-  by(inv_siteXsp['macro', , dimnames(inv_siteXsp)[[3]] %in% keep_taxa],
                     inv_plots$L2NAME, 
                     function(x) colSums(x)[colSums(x)>0])
...
fia_sp <- colSums(fia_siteXsp[, colnames(fia_siteXsp) %in% keep_taxa])
em_sp <- colSums(inv_siteXsp['epi_macro', , dimnames(inv_siteXsp)[[3]] %in% keep_taxa])
em_sp <- em_sp[em_sp > 0]
...
comp_splists <- list(fia = fia_sp[fia], inv = macro_sp[inv], both = data.frame(fia=fia_sp[both], inv=macro_sp[both]), 
		both_not_epi = data.frame(fia=fia_sp[both_not_epi], inv=macro_sp[both_not_epi]))

# Number of species shared across regions
comp_counts_reg <- apply(comp_splists_reg, 1:2, function(x) length(unlist(x)))
comp_counts_reg['both',] <- comp_counts_reg['both',]/2 # because the both entry has duplicate entries for each species
...
comp_counts['both'] <- comp_counts['both']/2 # because the both entry has duplicate entries for each species
comp_counts['both_not_epi'] <- comp_counts['both_not_epi']/2 # because the both_not_epi entry has duplicate entries for each species

prop_sp <- rbind(prop_sp, ALL=comp_counts[c('inv','both','fia')] / sum(comp_counts[c('inv','both','fia')]))

#write.csv(prop_sp, file.path(results_dir, 'proportion_species_across_datasets.csv'))

## Figure S2: Number of species in INV and FIA across regions
#pdf(file.path(fig_dir, 'SI', 'Fig_S2-compare_species_cross_region.pdf'), height=3, width=8.5)
...
inv_reg <- by(inv_siteXsp['macro', as.character(inv_plots$Site_Number), rownames(inv)], inv_plots$L2NAME, colSums)
inv <- cbind(inv, sapply(regions, function(reg) inv_reg[[reg]]))		

# Which ones are targeted by FIA (but not found)?
fia_ref <- read.csv(file.path(data_dir, "REF_LICHEN_SPECIES_2020-06-23.csv"))
...
fia_reg <- by(fia_siteXsp[fia_plots$yrplotid, rownames(fia)], fia_plots$L2NAME, colSums)
...
fia_data <- data.frame(fia_plots)
inv_data <- data.frame(inv_plots)
rownames(inv_data) <- inv_data$Site_Number
rownames(fia_data) <- fia_data$yrplotid

fia_data <- fia_data[, -ncol(fia_data)] # drop column added from spatial dataframe
inv_data <- inv_data[, -ncol(inv_data)] # drop column added from spatial dataframe
...
fia_comm <- fia_siteXsp[,colSums(fia_siteXsp)>=10]
focal_sp <- colnames(fia_comm)[colnames(fia_comm) %in% colnames(inv_m_comm)] # 14 species
...
fia_data <- cbind(fia_data, fia_sp[fia_data$yrplotid,])
inv_sp <- inv_m_comm[,focal_sp]; colnames(inv_sp) = focal_names
inv_data <- cbind(inv_data, inv_sp[as.character(inv_data$Site_Number),])
...
#pdf(file.path(fig_dir, 'SI/Fig_S4-species_distributions_singlevars.pdf'), height=18, width=18)
par(lend=1)
par(mfrow=c(7, length(xvars)))
par(mar=c(3, 3, 2.5, 1))
par(oma=c(2, 8, 0, 0))
...
for(sp in focal_names){
  for(x in xvars){
    ...
    # FIA
    make_plot(xrange, c(0,1), xlab=ifelse(counter %% 7 ==0, xvarnames[x], ''), cex=.8)
...
load(file.path(results_dir, "siteXspecies_matrices.RData"))

# Drop Genus-only taxa from both datasets unless there are no congenerics
keep_taxa <- get_species(c(colnames(fia_siteXsp), dimnames(inv_siteXsp)[[3]]))

# Make community data matrices
fia_comm <- fia_siteXsp[, colnames(fia_siteXsp) %in% keep_taxa]
inv_em_comm <- inv_siteXsp['epi_macro', , dimnames(inv_siteXsp)[[3]] %in% keep_taxa]
inv_em_comm <- inv_em_comm[, colSums(inv_em_comm)>0]
inv_e_comm <- inv_siteXsp['epi', , dimnames(inv_siteXsp)[[3]] %in% keep_taxa]
inv_e_comm <- inv_e_comm[,colSums(inv_e_comm)>0]


# Remove species occuring fewer than 3 times
# Remove sites where no species remain
dim(fia_comm) # 72 species originally, 174 plots
fia_comm <- fia_comm[, colSums(fia_comm)>2]; dim(fia_comm) # 35 species, lost 37
fia_comm <- fia_comm[rowSums(fia_comm)>0, ]; dim(fia_comm) # no sites lost


dim(inv_e_comm) # 260 species, 205 sites
inv_e_comm <- inv_e_comm[, colSums(inv_e_comm)>2]; dim(inv_e_comm) # 146 species, lost 114
inv_e_comm <- inv_e_comm[rowSums(inv_e_comm)>0,]; dim(inv_e_comm) # 193 plots, lost 12

dim(inv_em_comm) # 98 species, 205 sites
...
# None need to be excluded in any case
fact = 2.5

fia_dist = 1-vegdist(fia_comm, method='bray')
which(rowMeans(as.matrix(fia_dist)) > mean(fia_dist) + fact*sqrt(var(as.numeric(as.matrix(fia_dist)))))

inv_e_dist = 1-vegdist(inv_e_comm, method='bray')
which(rowMeans(as.matrix(inv_e_dist)) > mean(inv_e_dist) + fact*sqrt(var(as.numeric(as.matrix(inv_e_dist)))))

inv_em_dist = 1-vegdist(inv_em_comm, method='bray')
which(rowMeans(as.matrix(inv_em_dist)) > mean(inv_em_dist) + fact*sqrt(var(as.numeric(as.matrix(inv_em_dist)))))

## Perform NMDS and fit environmental variables to FIA data
# monoMDS engine, 20 random starts (or until stable solution found up to 300 random starts, with max of 500 iterations in a round)
# Try several dimensionalities

#fia_mds = metaMDS(fia_comm, distance='bray', k=3, autotransform=F, try = 20, trymax=300, maxit=500)
# dimension = 3, stress = 0.16228, 20 tries

#fia_mds = metaMDS(fia_comm, distance='bray', k=4, autotransform=F, try = 20, trymax=300, maxit=500)
# dimension = 4, stress = 0.12309, 76 tries

## USE THIS ONE
fia_mds = metaMDS(fia_comm, distance='bray', k=5, autotransform=F, try = 20, trymax=300, maxit=500)
# dimension = 5, stress = 0.0959, 20 tries

#fia_mds = metaMDS(fia_comm, distance='bray', k=6, autotransform=F, try = 20, trymax=300, maxit=500)
# dimension = 6, stress = 0.07810, 20 tries

#fia_mds = metaMDS(fia_comm, distance='bray', k=7, autotransform=F, try = 20, trymax=300, maxit=500)
# dimension = 7, stress = 0.06284, 30 tries

fia_ef = envfit(fia_mds, fia_data[rownames(fia_comm), c(xvars,'L2NAME')], choices=1:5)
...
#})
fia_ss = scores(fia_mds, 'sites', choices=1:5) # Oksanen suggests not doing this.
...
# dimension = 4, no convergence

#inv_e_mds = metaMDS(inv_e_comm, distance='bray', k=5, autotransform=F, trymax=300, maxit=500)
# dimension = 5, no convergence

# USE THIS ONE
inv_e_mds = metaMDS(inv_e_comm, distance='bray', k=6, autotransform=F, trymax=300, maxit=500)
# dimension = 6, stress = 0.1110, 126 tries

#inv_e_mds = metaMDS(inv_e_comm, distance='bray', k=7, autotransform=F, trymax=300, maxit=500)
# dimension = 7, stress = 0.0979, 88 tries

inv_e_ef = envfit(inv_e_mds, inv_data[rownames(inv_e_comm),c(xvars,'L2NAME')], choices=1:6)
...
#	list(mod12=mod12, mod34=mod34)
#})
inv_e_ss = scores(inv_e_mds, 'sites', choices=1:6) # Oksanen suggests not doing this.
...
inv_em_mds = metaMDS(inv_em_comm, distance='bray', k=5, autotransform=F, trymax=300, maxit=500)
# dimension = 5, no convergence

inv_em_mds = metaMDS(inv_em_comm, distance='bray', k=6, autotransform=F, trymax=500, maxit=1000)
# dimension = 6, no convergence
...
#save(fia_mds, fia_ef, inv_e_mds, inv_e_ef, inv_em_mds, inv_em_ef, file=file.path(results_dir, 'NMDS_results.RData'))
load(file.path(results_dir, 'NMDS_results.RData'))
...
#write.csv(compare_ef, file.path(results_dir, 'Table_3-compare_envfit_nmds.csv'))
#write.csv(compare_ef, file.path(results_dir, 'Table_S2-compare_envfit_nmds_subsample.csv'))


#### NO LONGER USING
#### the number of sites is similar across data sets
